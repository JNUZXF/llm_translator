...
class MarkdownToDocConverter:
    ...
    def convert_to_word(self, content: str) -> str:
        ...
        try:
            ...
            # 设置默认字体为楷体
            def set_font_kaiti(paragraph, font_size=12):
                """设置段落字体为楷体"""
            ...
            for element in soup.find_all(['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'p', 'ul', 'ol', 'table', 'img']):
                if element.name.startswith('h'):
                    ...
                    # 设置标题字体为楷体
                    set_font_kaiti(heading, font_size=16 if level <= 2 else 14)
                
                elif element.name == 'p':
                    # 段落
                    para = doc.add_paragraph(element.get_text())
                    set_font_kaiti(para, font_size=12)
                
                elif element.name in ['ul', 'ol']:
                    # 列表
                    for li in element.find_all('li'):
                        list_para = doc.add_paragraph(li.get_text(), style='List Bullet')
                        set_font_kaiti(list_para, font_size=12)
                    ...
                    try:
                        ...
                        if src and os.path.exists(src):
                            # 添加图片标题
                            caption_para = doc.add_paragraph(f"图片：{alt}")
                            set_font_kaiti(caption_para, font_size=10)
